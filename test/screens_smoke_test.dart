...
import 'package:vaultkey_app/main.dart';
...
class _MemoryProvider extends PasswordProvider {
  @override
...
Future<void> checkScreen(
  ...
  expect(
    ex,
    isNull,
    reason: '${name ?? screen.runtimeType} overflowed/errored:\n$ex',
  );
}
...
void main() {
  ...
  Future<void> runAll(WidgetTester tester) async {
    ...
    await checkScreen(
      tester,
      const OnboardingScreen(onNext: null),
      name: 'onboarding-1',
    );
    ...
    expect(
      ...
      isNull,
      reason: 'signup-validated (errors shown)',
    );
    ...
    await checkScreen(
      tester,
      DetailScreen(pw: mockPasswords.first),
      name: 'detail',
    );
    await checkScreen(
      tester,
      AddEditScreen(pw: mockPasswords.first),
      name: 'addEdit-edit',
    );
  ...
  testWidgets('full app boots to home without errors at phone size', (
    tester,
  ) async {
    await setPhone(tester);
    await tester.pumpWidget(
      ChangeNotifierProvider<PasswordProvider>(
        create: (_) => _MemoryProvider()..debugSetPasswords(mockPasswords),
        child: MaterialApp(
          debugShowCheckedModeBanner: false,
          theme: ThemeData(
            useMaterial3: true,
    ...
    await tester.pumpAndSettle();
    expect(tester.takeException(), isNull, reason: 'boot to home');
